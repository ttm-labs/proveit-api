...
PROTO_JS_OUTPUT_DIR=javascript
	...
	@for file in $(PROTO_INPUT_DIR)/*.proto; do \
		if [ -f "$$file" ]; then \
			...
			protoc \
				...
				--js_out="import_style=commonjs,binary:${PROTO_JS_OUTPUT_DIR}" \
				--grpc-web_out="import_style=typescript,mode=grpcwebtext:${PROTO_JS_OUTPUT_DIR}" \
				--ts_out="${PROTO_JS_OUTPUT_DIR}" \
				$$file; \
		fi; \
	done
